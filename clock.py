...
@sched.scheduled_job('interval', seconds=5, max_instances=1)
def pollNode():
...
@sched.scheduled_job('interval', seconds=120, max_instances=1)
def syncBlockchain():
    ...
    for i in range(lastTrustedBlock+1, currBlock):
        if not i in hash_set:
            ...
            q.enqueue(storeBlockInDB, i, nodeAPI, timeout=1800)
            stopTrust = True
        if not stopTrust:
            newLastTrusted = i
